...
def read_xlm(task_flag):
    ...
    if task_flag == '1':
        print(
            '---------------------------------Import list of video info from excel file----------------------------\n\n')
        ...
        print(
            '---------------------------------Import list of transcipt info from excel file----------------------------\n\n')
        ...
        print(
            '---------------------------------Import list of thumbnails info from excel file----------------------------\n\n')
...
def upload_video(videos):
    for video in videos:
        ...
        upload_command_template = 'cd video_source & python video_source/upload_video.py --file=' + filename_template
        if video['title'] != "":
...
def upload_transcript(transcripts):
    for transcript in transcripts:
        ...
        upload_command_template = 'cd video_source & python upload_caption.py --videoid=' + str(
            transcript['videoid']) + ' --file=' + filename_template
        ...
        print("---------------------------------start uploading " + transcript[
            'filename'] + "---------------------------------")
        print(upload_command_template)
        os.system(upload_command_template)
        print(
...
def upload_thumbnail(thumbnails):
    for thumbnail in thumbnails:
        ...
        upload_command_template = 'cd video_source & python upload_thumbnails.py --file ' + filename_template + ' --video-id=' + str(
            thumbnail['videoid'])
        print(str(thumbnail['videoid']))
        print("---------------------------------start uploading thumbnail:  " + thumbnail[
            'filename'] + "---------------------------------")
        print(upload_command_template)
        os.system(upload_command_template)
        print(
...
def output_video_list(title, youtube_id):
    ...
    file.write(
        datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ' , ' + title + ' , ' + 'https://youtu.be/' + youtube_id + '\n')
    file.close()
